...
public class _CardGameManager : MonoBehaviour
{
    ...
    private void SpriteCardAllocation()
    {
		...
		int[] selectedID = new int[totalPairs];

		// Asegurarse de que no se repitan imágenes si hay más cartas que imágenes disponibles
		if (totalPairs > sprites.Length)
		{
			Debug.LogError("No hay suficientes sprites para crear pares únicos.");
			return;
		}
		...
		// Shuffle selectedID array to randomize sprite pairs
		for (i = 0; i < totalPairs; i++)
		{
			int temp = selectedID[i];
			int randomIndex = Random.Range(i, totalPairs);
			selectedID[i] = selectedID[randomIndex];
			selectedID[randomIndex] = temp;
		}
	...
	private void Update(){
		if (gameStart) {
			time += Time.deltaTime;
			timeLabel.text = "Tiempo: " + Mathf.Round(time * 100f) / 100f + "s";
		}
	}
